...
export function ProfileCard({
  ...
  const handleFollowClick = async () => {
    ...
    try {
      ...
      if (!data.success) {
        ...
        setIsFollowing(wasFollowing);
        setFollowersCount((prev) =>
          wasFollowing ? prev + 1 : Math.max(prev - 1, 0),
        );
  ...
  return (
    <div className="w-full animate-fade-in">
      ...
      {/* Action button — only show after follow status is loaded to prevent flash */}
      {followStatusLoaded ? (
        isOwnProfile ? (
          <Button
        ...
        /* Skeleton placeholder while follow status loads */
        <div className="w-full mt-4 h-10 rounded-lg bg-secondary/50 animate-pulse" />
      )}
